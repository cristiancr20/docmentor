...
- **Verificar la app en navegador sin sesión interactiva (frontend):** `playwright-cli` (skill `playwright-cli`) contra el backend local. El backend Strapi exige Node `<=20` (`node` global es 26 y revienta en `buffer-equal-constant-time`): arrancarlo con `PATH=$HOME/.nvm/versions/node/v20.20.2/bin:$PATH npm run develop` desde `backend/`; su `package-lock.json` solo trae `@swc/core-linux-x64-gnu`, así que en Mac arm64 hace falta `npm install --no-save --no-package-lock @swc/core-darwin-arm64@<versión de @swc/core>` (no toca archivos versionados). Esperar el 204 de `/_health`. CORS del backend (`config/middlewares.js`) solo admite `localhost:3000` y `:3001`: para probar el build de producción con login usar `npx vite preview --port 3001`, no el 4173 por defecto. Selectores útiles del visor: `.rpv-core__viewer`, `.rpv-core__canvas-layer canvas`, `.rpv-core__text-layer span` (seleccionar texto con `page.mouse` down/move/up para que aparezca el botón "Comentar"), `.rpv-core__inner-pages` (el elemento que hace scroll, para verificar el scroll sincronizado). El worker se comprueba con `performance.getEntriesByType('resource')` filtrando `/pdf.worker.js` y `responseStatus`. Datos de prueba: proyecto 6 (tutor id 2, estudiante id 1), documento 7 con 2 comentarios. Borrar de `backend/.tmp/data.db` los comentarios creados al probar (tablas `comments`, `comments_documents_links`, `comments_correction_tutor_links`) y restaurar `documents.is_revised` si se alteró.
...
## 2026-09-17 - US-006
- Verificación en navegador (playwright-cli/Chromium) de la migración a Vite, en dev (`npm start`, :3000) y con el build de producción (`vite preview`), contra Strapi local con los usuarios del seed. Todo funciona igual que antes; **no hubo que cambiar ningún archivo de código**.
- Login `tutor@docmentor.local` → `/tutor/dashboard` y `estudiante@docmentor.local` → `/student/dashboard`: 0 errores de consola (en dev solo los 2 avisos de future flags de React Router, previos a la migración; en producción 0 mensajes).
- Visor `/document/7`: `/pdf.worker.js` 200 y `/uploads/*.pdf` 200; 1 página con canvas y capa de texto (52 spans); los 2 resaltados existentes pintados; creación de comentario nuevo (selección de texto → botón "Comentar" → formulario → guardado) con su resaltado. Igual en dev y producción.
- Comparador `/project/6`: pestaña "Cambios" muestra `% de contenido en común` y diff antes/después; "Lado a lado" carga 2 `.rpv-core__viewer` con resaltados rojo/verde/amarillo; scroll sincronizado proporcional (1500/4920 izq → 247/809 der). Igual en dev y producción.
- Recarga directa de `/tutor/dashboard` con `vite preview`: 200 `text/html` con la app (curl en :4173 y navegador en :3001); `dist/index.html` referencia `/assets/*` y `dist/pdf.worker.js` existe.
- Archivos cambiados: solo `tasks/prd-vite.json` (notes) y `.ralph-tui/progress.md`. Datos de prueba creados durante la verificación borrados de `backend/.tmp/data.db`.
- Checks: `npm test` ✅ (15 archivos, 86 tests), `npm run build` ✅, `npm run typecheck` ✅, `npx eslint 'src/**/*.{js,jsx}'` ✅.
- **Learnings:**
  - Ver patrón "Verificar la app en navegador sin sesión interactiva" en Codebase Patterns (Node 20 + binario swc para el backend, CORS solo en :3000/:3001, selectores del visor).
  - El AC pedía probar en `http://localhost:4173`, pero el CORS del backend (intocable según el PRD) rechaza ese origen: el login falla con `No 'Access-Control-Allow-Origin'`. Se sirvió el mismo `dist/` con `npx vite preview --port 3001`; en :4173 se validaron por curl assets, worker y fallback SPA. Si algún día se quiere usar 4173 hay que añadirlo al `origin` de `backend/config/middlewares.js`.
  - `vite preview` ya hace el fallback SPA (`appType: 'spa'` por defecto), así que las rutas profundas responden la app sin configuración extra; `vercel.json` (US-004) cubre lo mismo en Vercel.
  - Al añadir un comentario, el documento pasa de "Revisado" a "Pendiente" (`is_revised` 1 → 0): es comportamiento de la app, no de la migración.
  - El `[LOG] Warning: Invalid PDF header version: 1.3%` que emite `pdf.worker.js` en el comparador viene de un PDF subido con cabecera malformada, no de Vite (el worker es el mismo archivo de `public/`).
---
